...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "webmessenger" because\n';
  ...
  $wnd.__gwt_activeModules['webmessenger'] = {
    'moduleName' : 'webmessenger',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/webmessenger.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['webmessenger'].superdevmode = true;

  var expectedSuffix = '/webmessenger.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + webmessenger + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:webmessenger';
  var devModeSessionKey = '__gwtDevModeSession:webmessenger';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/webmessenger/webmessenger.nocache.js';
  ...
  $wnd.__gwt_activeModules['webmessenger'].canRedirect = true;
  $wnd.__gwt_activeModules['webmessenger'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING webmessenger';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/webmessenger?user.agent=' + ua + '&_callback=' + callback;
